# Tutorial: Introduction to Spatial Data

You will not turn in this tutorial. This tutorial is designed as an ungraded contact-time activity that allows you to gain the skills necessary to complete HW#2. 

In this tutorial, you will practice working with spatial data formats using the `sf` (for vector data),  `sfdep` library (spatial statistics), the `terra` package (for raster data), and `tmap` (for mapping) packages. After completing these exercises, you should be able to:
...
You will use the following datasets (you do not need to download, these are just links to the original source):
...
Add a new first level header called "Mini-Challenge". Under that header, add a code chunk. In the code chunk, write commands to do the following:
